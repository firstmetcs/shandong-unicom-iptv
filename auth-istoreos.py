...
class IPTVAuthenticator:
    ...
    def generate_authenticator(self, encrypt_token):
        ...
        plaintext = f"{random_num}${encrypt_token}${self.config['user_id']}${self.config['stb_id']}${self.config['ip']}${self.config['mac']}$Reserved${self.config['platform']}"
        # DES加密（3DES退化为DES）
    ...
    def step4_1_generate_m3u(self, html):
        ...
        with open(save_dir_m3u, 'w', encoding='utf-8') as fm3u:
            ...
            for channel in channels:
                ...
                # 支持时移的M3U标签
                url=f'rtp://{igmp}'
                m3u_ts = f' catchup="default" catchup-source="{ts_url}&playseek=${{(b)yyyyMMddHHmmss}}-${{(e)yyyyMMddHHmmss}}"' if timeshift == '1' and ts_url else ""
